...
fn main() {
    ...
    // Minute-specific patch (see src-tauri/Cargo.toml's `[patch.crates-io]`
    // comment for the full rationale): upstream links this unconditionally
    // via `cargo:rustc-link-lib=framework=ScreenCaptureKit`, which emits a
    // hard `LC_LOAD_DYLIB` — dyld refuses to even launch the process if the
    // framework isn't present (ScreenCaptureKit.framework doesn't exist
    // before macOS 12.3). Minute's declared floor is macOS 11.0 with only a
    // *runtime* feature gate for system-audio capture (see syscap.rs), so a
    // hard link here would silently break every other feature of the app on
    // macOS <12.3, not just system audio. `-weak_framework` instead emits
    // `LC_LOAD_WEAK_DYLIB`: dyld resolves it lazily and leaves it unbound
    // if absent, so `screencapturekit`'s own symbols are only ever touched
    // by the (already macOS-13-gated) code paths that call them, and every
    // other symbol from every other framework this binary links loads
    // normally regardless of OS version. Verified empirically with a
    // throwaway `-weak_framework`-linked binary + `otool -l` showing
    // `LC_LOAD_WEAK_DYLIB` (not `LC_LOAD_DYLIB`) for the entry.
